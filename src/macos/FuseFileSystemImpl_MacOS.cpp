...
MountId FuseFileSystemImpl_MacOs::mount(
    FileRenderOptions options, int draftScale, const std::string& srcFile, const std::string& dstPath, const CalibrationProfile* calibration)
{
...
void FuseFileSystemImpl_MacOs::updateOptions(MountId mountId, FileRenderOptions options, int draftScale, const CalibrationProfile* calibration) {
    auto it = mMountedFiles.find(mountId);
    if(it != mMountedFiles.end()) {
        it->second->updateOptions(options, draftScale, calibration);
    }
}
