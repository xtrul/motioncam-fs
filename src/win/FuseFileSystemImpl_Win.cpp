...
public:
    void updateOptions(FileRenderOptions options, int draftScale, const CalibrationProfile* calibration);
...
void Session::updateOptions(FileRenderOptions options, int draftScale, const CalibrationProfile* calibration) {
    mOptions = options;
    mDraftScale = draftScale;

    // Tell file system about new options
    mFs->updateOptions(options, draftScale, calibration);
...
MountId FuseFileSystemImpl_Win::mount(FileRenderOptions options, int draftScale, const std::string& srcFile, const std::string& dstPath, const CalibrationProfile* calibration) {
    ...
    if(boost::iequals(extension, ".mcraw")) {
        ...
        try {
            auto fs = std::make_unique<VirtualFileSystemImpl_MCRAW>(*mIoThreadPool, *mProcessingThreadPool, *mCache, options, draftScale, srcFile, calibration);
...
void FuseFileSystemImpl_Win::updateOptions(MountId mountId, FileRenderOptions options, int draftScale, const CalibrationProfile* calibration) {
    auto it = mMountedFiles.find(mountId);
    if(it == mMountedFiles.end())
        return;

    dynamic_cast<Session*>(mMountedFiles[mountId].get())->updateOptions(
        options, draftScale, calibration);
}
